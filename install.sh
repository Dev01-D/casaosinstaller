#!/usr/bin/bash
#
#           CasaOS Installer Script v0.4.16#
...
Install_Docker() {
    ...
    # NOTE:
    # We intentionally do NOT pin Docker to 24.0.7 anymore.
    # This avoids failures on newer distros (e.g. Debian 13 / trixie)
    # where that exact version is not available from the Docker repo.
    if [[ "${REGION}" = "China" ]] || [[ "${REGION}" = "CN" ]]; then
        # Use the Aliyun mirror but let the script pick a suitable version
        ${sudo_cmd} curl -fsSL https://play.cuse.eu.org/get_docker.sh | bash -s docker --mirror Aliyun
    else
        # Standard Docker installation without version pinning
